...
func writeModuleTable(moduleList []string, librarySymbolDictionary *OrderedMap, segmentData *[]byte) uint64 {
    ...
    for _, library := range libraries {
        ...
        if contains(runningModuleList, libraryStr) {
            continue
        }
...
func writeNIDTable(orbisElf *OrbisElf, segmentData *[]byte) (uint64, error) {
    ...
    for _, symbol := range symbols {
        ...
        for libraryIndex, library := range libraries {
            librarySymbols := orbisElf.LibrarySymbolDictionary.Get(library)
            // This occurs if...s
            if librarySymbols == nil {
                continue
            }

            if contains(librarySymbols.([]string), symbol.Name) {
        ...
        for libraryIndex, library := range modules {
        ...
        nidTableBuff.WriteString(buildNIDEntry(symbol.Name, 1+symbolModuleIndex, 1+symbolLibraryIndex))
    }
